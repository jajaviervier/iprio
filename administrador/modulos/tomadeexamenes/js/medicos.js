rutaMedicos = "sistema/TomadeExamenes/"
...
function eliminarRecepcionista(llave) {
    alertify.confirm('Eliminar Usuario', 'Está seguro que desea eliminar el usuario?', function() {
        ...
        if (estadoMedico == false) {
            if (passMedico == rePassMedico) {
                secondaryApp.auth().createUserWithEmailAndPassword(correoMedico, passMedico).then(function(firebaseUser2) {
                    ...
                    Materialize.toast('<span class="green-text">Usuario Registrado Con Exito!</span>', 4000)
                })
            } else {

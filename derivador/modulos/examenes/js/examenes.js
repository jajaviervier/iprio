...
function cargarExamenesPendientes() {
    db.ref(rutaExamenes).orderByChild('llaveUsuarioSolicitante').equalTo(firebase.auth().currentUser.uid).on('value', function(datExamenPendiente) {
        contenidoTablaExamenPendiente = ''
        console.log('ssdds')
        datExamenPendiente.forEach(function(iExamenPendiente) {
            datexampend = iExamenPendiente.val();
            console.log(datexampend)
            contenidoEstado = "";
            if (datexampend.estado == "Pendiente") {
...
<td> ` + contenidoEstado + `<i class="material-icons" onclick="abrirOpcionesExamen('` + iExamenPendiente.key + `')">
                                    content_paste
                                </i> </td>
            `
            }
...
function abrirOpcionesExamen(llave) {
    ...
    db.ref(rutaExamenes).orderByKey().equalTo(llave).once('value', function(datExamenPendiente) {
        datExamenPendiente.forEach(function(iExamenPendiente) {
            ...
            if (dd.estadoArchivo == "ninguno") {
                $('#collectionArchivo').html(`
             
                <div class="row white">

                <div class="col s1s"> 
No existe Archivo vinculado.
                </div>
 <div class="file-field input-field col s12 ">
                    <div class="btn">
                     <i class="material-icons" >
                        ...
                        <input id="cargadorArchivo" type="file">
                        </input>
                    </div>
         ...
         <div class="col s12"> 
<input type="button" class="btn" value="Subir" onclick="cargarArchivo()">
                </div>
                </div>
...
function cargarExamenesFinalizados() {
    db.ref(rutaExamenes).orderByChild('llaveMedico').equalTo(firebase.auth().currentUser.uid).once('value', function(datExamenPendiente) {
        contenidoTablaExamenPendiente = ''
        datExamenPendiente.forEach(function(iExamenPendiente) {
            datexampend = iExamenPendiente.val();
            contenidoEstado = "";
            if (datexampend.estado == "Finalizado") {
                contenidoTablaExamenPendiente += `<tr>
<td> ` + datexampend.nombreMedico + `  ` + datexampend.apellidoMedico + `</td>
<td> ` + datexampend.nombrePaciente + `  ` + datexampend.apellidoPaciente + `</td>
<td> ` + datexampend.siglaExamen + ` </td>
...
function guardarSolicitudExamen(medico, paciente, estudio) {
    ...
    if (estadoFormularioExamen == true) {
        db.ref('sistema/examenes').push({
            ...
            tipoUsuarioRegistro: "derivador",
            llaveUsuarioSolicitante: firebase.auth().currentUser.uid,
            ...
            Materialize.toast('<span class="green-text">Datos guardados con exito!</span>', 4000)
            db.ref(rutaNotificaciones).once('value', function(datNoti) {
                sumaNoti = 0
